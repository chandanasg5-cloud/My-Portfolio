    ...
    <section id="contact" style={{ padding: '5rem 2rem', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--muted)', fontSize: '0.9375rem', marginBottom: '2.5rem', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            {[
              ...
              <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 8, padding: '0.65rem 1.2rem', transition: 'all 0.2s' }}
                ...
                onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = 'var(--border)'; el.style.color = 'var(--text)'; el.style.background = 'transparent' }}
              >
                {icon} {label}
              </a>
            ))}
