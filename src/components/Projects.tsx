      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
          {projects.map((project) => (
            <div key={project.title} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: '1.75rem', transition: 'box-shadow 0.2s, border-color 0.2s' }}
            ...
            >
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--muted)', lineHeight: 1.7, marginBottom: '1rem' }}>{project.description}</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {project.tags.map(tag => (
                  <span key={tag} style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', fontSize: '0.72rem', color: 'var(--muted)', padding: '0.2rem 0.55rem', borderRadius: 4, fontWeight: 500 }}>
                    {tag}
                  </span>
                ))}
              </div>
